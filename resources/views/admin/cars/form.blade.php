...
@endif
@if(!$car)
<div class="card" style="margin-bottom:18px;border:1.5px solid #e0e7ff;background:linear-gradient(135deg,#f8faff 0%,#eef2ff 100%)">
    ...
    <div style="display:flex;gap:8px">
        ...
        <button type="button" id="otomotoImportBtn" class="btn btn-blue" style="white-space:nowrap;padding:10px 20px" onclick="importFromOtomoto()">
...
<script>
// ===== Otomoto Import =====
async function importFromOtomoto(){
    ...
    try {
        ...
        if(!result.success){
            showImportStatus(result.message || 'Nie udało się zaimportować danych.', 'error');
            return;
        }
        ...
        // Brand — match by name to existing select options
        if(d.brand){
        ...
        // Select dropdowns (fuel_type, transmission, body_type)
        const selectMap = {
        ...
        showImportStatus(`✓ Zaimportowano ${filled} pól z Otomoto. Sprawdź dane i uzupełnij brakujące.`, 'success');

    } catch(err){
        showImportStatus('Błąd sieci — spróbuj ponownie. ' + err.message, 'error');
    } finally {
        btn.disabled = false;
