...
/* 4×4 grid — layout wg reference. Komórki NIE są osobnymi kafelkami z
   tłem: cała siatka to jedna tabela rozdzielona włoskowymi liniami
...
/* Ostatnia kolumna bez prawej linii, ostatni wiersz bez dolnej — inaczej
   siatka ma wiszące krawędzie przy krawędzi karty. */
...
@media(max-width:1024px){
    .cs-dp-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
    .cs-dp-grid > .cs-dp-item{border-right:1px solid #eef1f6;border-bottom:1px solid #eef1f6}
    .cs-dp-grid > .cs-dp-item:nth-child(2n){border-right:none}
    .cs-dp-grid > .cs-dp-item:nth-last-child(-n+2){border-bottom:none}
}
@media(max-width:520px){
    .cs-dp-grid{grid-template-columns:1fr}
    .cs-dp-item{padding:14px 4px;border-right:none!important}
    .cs-dp-grid > .cs-dp-item{border-bottom:1px solid #eef1f6}
    .cs-dp-grid > .cs-dp-item:last-child{border-bottom:none}
...
<div class="container" style="padding-top:16px">
    ...
    <div class="cs-data-section cs-collapsible-mobile">
        ...
        <div class="cs-data-body">
            ...
            @php
                ...
                $em = '—';
                // Wersja (poziom wyposażenia) i Silnik to wg reference DWA
                // osobne pola — wcześniej jedna komórka "Wersja / silnik".
                ...
                // teraz stałą komórkę w siatce wyżej (wg reference).
    ...
    {{-- =================== 3-CARD SUMMARY ROW (Historia / Dokumenty / Serwisowanie) =================== --}}
    @php
        // Prefer the admin-entered free-text service_history; fall back to the
        // Tak/Nie flags only when the longer description isn't set.
        $svc3      = $rowOk($car->service_history)
            ? $car->service_history
            : (CarLabels::bool($car->service_book) ?: CarLabels::bool($car->service_documentation));
        ...
        $hasMuted = fn($v) => $v ?: '—';
        // Heuristics for the right-most "Serwisowanie" card. The new admin
        // fields take precedence; older rows fall back to the closest legacy
        // field so existing inventory still renders sensibly.
        $svcConfirmation = $rowOk($car->service_confirmation_type)
            ? $car->service_confirmation_type
            : ($bookSt3 ?: ($svc3 ?: null));
    ...
    {{-- The Serwisowanie / Dokumenty / Zużycie paliwa 3-card row was removed
         here — Serwisowanie + Dokumenty are now in the upper 3-card summary
         (see "3-CARD SUMMARY ROW" above), matching the reference 1:1. --}}
