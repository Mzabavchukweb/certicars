...
<head>
    ...
    <title><?php echo $__env->yieldContent('title','Panel admina'); ?> — CertiCars</title>
...
<div class="ad-wrap">
    <aside class="ad-side" id="adSide">
        ...
        <nav class="ad-nav">
            ...
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="<?php echo e(request()->routeIs('admin.dashboard')?'active':''); ?>"><i data-lucide="layout-dashboard"></i> Dashboard</a>
            <a href="<?php echo e(route('admin.messages.index')); ?>" class="<?php echo e(request()->routeIs('admin.messages.*')?'active':''); ?>">
                ...
                <?php if(($unreadMessagesCount ?? 0) > 0): ?><span class="badge"><?php echo e($unreadMessagesCount); ?></span><?php endif; ?>
            </a>
            <div class="section-lbl">Zarządzanie</div>
            <a href="<?php echo e(route('admin.cars.index')); ?>" class="<?php echo e(request()->routeIs('admin.cars.*')?'active':''); ?>"><i data-lucide="car"></i> Samochody</a>
            <a href="<?php echo e(route('admin.brands.index')); ?>" class="<?php echo e(request()->routeIs('admin.brands.*')?'active':''); ?>"><i data-lucide="tag"></i> Marki</a>
            <div class="section-lbl">Konto</div>
            <a href="<?php echo e(route('admin.profile.edit')); ?>" class="<?php echo e(request()->routeIs('admin.profile.*')?'active':''); ?>"><i data-lucide="user"></i> Profil</a>
            ...
            <a href="<?php echo e(route('home')); ?>" target="_blank"><i data-lucide="external-link"></i> Strona główna</a>
            <a href="<?php echo e(route('catalog')); ?>" target="_blank"><i data-lucide="external-link"></i> Oferta</a>
        </nav>
        <div class="ad-side-bottom">
            <div class="ad-user">
                <span class="av"><?php echo e(strtoupper(substr(auth()->user()?->name ?? 'A', 0, 1))); ?></span>
                <div class="ad-user-info">
                    <div class="n"><?php echo e(auth()->user()?->name ?? 'Admin'); ?></div>
                    <div class="e"><?php echo e(auth()->user()?->email); ?></div>
                </div>
            </div>
        ...
        <div class="ad-topbar">
            ...
            <div class="ad-topbar-left">
                <div>
                    ...
                    <?php if (! empty(trim($__env->yieldContent('crumbs')))): ?><div class="crumbs"><?php echo $__env->yieldContent('crumbs'); ?></div><?php endif; ?>
                </div>
            </div>
            ...
            <div class="ad-topbar-actions"><?php echo $__env->yieldContent('actions'); ?></div>
        </div>
...
// ==== Command palette ====
const CMDK_PAGES=[
    {t:'Dashboard',s:'Przegląd',icon:'layout-dashboard',url:'<?php echo e(route('admin.dashboard')); ?>',kbd:'g d'},
    {t:'Samochody',s:'Lista ofert',icon:'car',url:'<?php echo e(route('admin.cars.index')); ?>',kbd:'g c'},
    {t:'Dodaj samochód',s:'Nowa oferta',icon:'plus',url:'<?php echo e(route('admin.cars.create')); ?>',kbd:'n'},
    {t:'Wiadomości',s:'Skrzynka odbiorcza',icon:'inbox',url:'<?php echo e(route('admin.messages.index')); ?>',kbd:'g m'},
    {t:'Nieprzeczytane wiadomości',s:'Filtr',icon:'mail',url:'<?php echo e(route('admin.messages.index',['filter'=>'unread'])); ?>'},
    {t:'Marki',s:'Zarządzanie markami',icon:'tag',url:'<?php echo e(route('admin.brands.index')); ?>',kbd:'g b'},
    {t:'Profil',s:'Dane konta, hasło',icon:'user',url:'<?php echo e(route('admin.profile.edit')); ?>'},
    {t:'Strona główna',s:'Widok publiczny',icon:'external-link',url:'<?php echo e(route('home')); ?>',external:true},
    {t:'Oferta publiczna',s:'Katalog',icon:'external-link',url:'<?php echo e(route('catalog')); ?>',external:true},
...
function cmdkRun(it){
    if(!it)return;
    closeCmdk();
    if(it.logout){document.querySelector('form[action="<?php echo e(route('logout')); ?>"]')?.submit();return}
...
async function cmdkSearchCars(q){
    try{
        const r=await fetch(`<?php echo e(route('admin.cars.index')); ?>?search=${encodeURIComponent(q)}&format=json`,{headers:{Accept:'application/json'}});
        if(!r.ok)return[];
...
document.addEventListener('keydown',e=>{
    ...
    if(e.key.toLowerCase()==='n'){window.location='<?php echo e(route('admin.cars.create')); ?>';return}

    if(gSeq){
        clearTimeout(gSeq.t);
        const k=e.key.toLowerCase();
        const map={d:'<?php echo e(route('admin.dashboard')); ?>',c:'<?php echo e(route('admin.cars.index')); ?>',m:'<?php echo e(route('admin.messages.index')); ?>',b:'<?php echo e(route('admin.brands.index')); ?>'};
...
<?php if(session('success')): ?>toast(<?php echo json_encode(session('success'), 15, 512) ?>,'success');<?php endif; ?>
<?php if(session('error')): ?>toast(<?php echo json_encode(session('error'), 15, 512) ?>,'error');<?php endif; ?>
<?php if($errors->any()): ?>toast(<?php echo json_encode($errors->first(), 15, 512) ?>,'error','Błąd walidacji');<?php endif; ?>
...
<?php /**PATH /Users/maksymzabavchuk/Desktop/certicars/resources/views/admin/layouts/app.blade.php ENDPATH**/ ?>