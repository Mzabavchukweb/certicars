...
{{-- ============================================================
     Otomoto Import (create only)
     ============================================================ --}}
@if(!$car)
<div class="wz-otomoto-card">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
        ...
        <div>
            ...
            <div style="font-size:12px;color:#6366f1">Wklej link ogłoszenia — dane wypełnią się automatycznie</div>
        </div>
    </div>
    <div style="display:flex;gap:8px">
        <input type="text" id="otomotoUrl" placeholder="https://www.otomoto.pl/osobowe/oferta/..." style="flex:1;padding:10px 14px;border:1px solid #c7d2fe;border-radius:8px;font-size:13px;background:#fff">
        <button type="button" id="otomotoImportBtn" class="btn btn-blue" style="white-space:nowrap;padding:10px 20px" onclick="wzImportFromOtomoto()">
            <i data-lucide="download" style="width:14px;height:14px"></i> Importuj
        </button>
    </div>
    <div id="otomotoStatus" style="margin-top:8px;font-size:12px;display:none"></div>
</div>
@endif
...
<div class="wz-step" data-step="2">
    ...
    {{-- 360° exterior panorama --}}
    <div class="wz-media-section">
        <div class="wz-media-header">
            <i data-lucide="scan"></i>
            <h3>Panorama 360° — zewnętrze</h3>
        </div>
        <p style="font-size:12px;color:var(--text-3);margin:0 0 14px">Jedno zdjęcie equirectangular obejmujące pełny widok zewnętrzny dookoła pojazdu.</p>
...
(function(){
    ...
    //  OTOMOTO IMPORT
    // ===================================================================
    window.wzImportFromOtomoto = async function(){
        const urlInput = document.getElementById('otomotoUrl');
        const btn = document.getElementById('otomotoImportBtn');
        const status = document.getElementById('otomotoStatus');
        ...
        btn.disabled = true;
        btn.innerHTML = '<i data-lucide="loader-2" style="width:14px;height:14px;animation:spin 1s linear infinite"></i> Importuję...';
        ...
        try {
            ...
            // Simple fields
            const fieldMap = {
                'model': d.model, 'price': d.price, 'currency': d.currency || 'PLN',
                'first_registration': d.first_registration, 'mileage': d.mileage,
                'engine_capacity': d.engine_capacity, 'power_hp': d.power_hp,
                'color': d.color, 'doors': d.doors, 'seats': d.seats, 'vin': d.vin,
                'co2_emission': d.co2_emission, 'country_registration': d.country_registration,
                'previous_owners': d.previous_owners, 'category': d.body_type,
            };
            ...
            // Select dropdowns
            const selectMap = { 'fuel_type': d.fuel_type, 'transmission': d.transmission, 'body_type': d.body_type };
            ...
            // Equipment — fill comfort textarea with all items
            if (d.equipment && Array.isArray(d.equipment) && d.equipment.length) {
                const eqTextarea = document.querySelector('textarea[name="equipment[comfort]"]');
                if (eqTextarea) {
                    eqTextarea.value = d.equipment.join('\n');
                    eqTextarea.dispatchEvent(new Event('input', {bubbles:true}));
                    filled++;
                }
            }
    ...
    function wzShowImportStatus(msg, type){
        const el = document.getElementById('otomotoStatus');
        if (!el) return;
        el.style.display = '';
